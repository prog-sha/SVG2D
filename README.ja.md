...
SVG2D は、SVG を Godot の 2D・3D シーンに表示するための軽量な GDExtension アドオンだよ。2Dでは `SVG2D`、3Dでは `SVG3D`、PBD紐には `SVGRope2D` / `SVGRope3D` ノードを使おう。画面上の描画領域を整数画素で見て解像度を更新し、同じ画素数の間は画像を使い回すよ。
