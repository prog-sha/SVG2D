...
SVG2D は、SVG を Godot の 2D・3D シーンに表示するための軽量な GDExtension アドオンだよ。画像素材のPBD紐には `SpriteRope2D` / `SpriteRope3D`、SVG素材には別クラスの `SVGRope2D` / `SVGRope3D` を使おう。画面上の描画領域を整数画素で見て解像度を更新し、同じ画素数の間は画像を使い回すよ。
