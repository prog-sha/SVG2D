// 形を画素の濃さに変えるところ。
//
// 責務: 閉じた形の並びを受けて、画素ごとの「どの程度覆われているか」を出すこと。
...
// 置きかたはここで掛ける。先に写して動かした形を作ると、札ごとに点の数に応じて
// 入れ物を取り直すことになる
...
// 形の入る四角。足し込む前に、どの程度の入れ物が要るかを知るのに使う。
...
// 掛け合わせ済みで持つと、まとまりごとの薄さを重ねるときにそのまま掛ければ済む。
// 8 bit で持つのは、Chromium が同じ持ちかたをしているからと、実数で持つと
// 800x800 で 10 MB になり、その置き場を用意する時点で 4 ミリ秒かかるため。
struct Canvas {
	int w = 0, h = 0;
	std::vector<uint8_t> px;   // w * h * 4
	// 触った範囲。まとまりの薄さを重ねるときに、この範囲を見れば済む
	...
	// 触った範囲を 0 へ戻す。深さごとに紙を使い回すのに使う。
	// 札ごとに作り直すと、800x800 で 2.5 MB を用意して埋めるのが札の数に応じて起きる
	void clear() {
	...
	// 計算は実数でして、置く場合に 8 bit へ丸める（Skia と同じ）
