...
SVG2D は、SVGをGodotの中で描き、そのまま2D・3Dシーンへ使うためのアドオンだよ。上部の **SVG** 画面で標準SVGファイルを開いて保存できるよ。実行時は画面上の描画領域に合わせて画像を使い回し、`SVG3D` は投影画素と同じ寸法で画像化して斜め・遠方表示向けのミップマップも作るよ。
...
`adaptive` は初めから有効だよ。実行中のカメラに加えて3Dエディターのカメラにも追従するよ。SVG文書の自然解像度に固定したいときは無効にしよう。この場合も各辺の上限は4096画素だよ。
