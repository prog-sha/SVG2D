...
| --- | --- |
| Asset Name | SVG2D |
| Description | SVG2D adds a lightweight Node2D that rasterizes SVG markup only when its requested size changes, then reuses the generated texture. It supports shapes, paths, fills, strokes, gradients, clipping paths, use elements, nested SVG elements, viewBox, and basic CSS selectors. This release includes Windows x86_64 binaries and requires Godot 4.7 or later. |
| Category | 2D Tools |
| License | MIT |
...
The GitHub provider can be used because the precompiled DLLs are tracked in the repository. The `godot-cpp` submodule is needed only to build the extension from source and is not needed by Asset Library users.

Before submitting:

1. Build and test both Windows x86_64 binaries with the release source.
